...
import {PanelAction} from '../components/og-panel';
...
import {PanelPeakUsageUdg} from './panel-peak-usage-udg';
...
@customElement('panel-peak-notification-udg')
export class PanelPeakNotificationUdg extends PanelPeakUsageUdg {
    ...
    public heading = html`<or-translate value="panel_peakNotification_udg.heading"></or-translate>`;
    public dark = true;
    //*public action: PanelAction = this.getDefaultAction();  //* comentada para NO dibujar el botón alargado para unirse al reto

    protected getDefaultAction(): PanelAction {
        return {
            text: 'panel_peakNotification_udg.buttonText', // uses auto-translate
    ...
    protected willUpdate(changedProps: PropertyValues) {
        ...
        if(changedProps.has('status')) {
            ...
            // Show a "SUCCESS" button for 3 seconds, and then change it to countdown.
                ...
                this.setActionVisual({text: i18next.t('panel_peakNotification.joinedText'), color: 'var(--og-color-success)', disabled: true});
                if(this.challengeAsset) {
                    setTimeout(() => this.startActionCountdown(this.challengeAsset), Defaults.PEAK_NOTIFICATION_JOINED_TIMEOUT_MS);
                }
            }
            ...
            // since we do not expect this panel to be shown during a challenge.
                ...
                this.setActionVisual({text: i18next.t('panel_peakNotification_udg.startingSoon'), color: 'var(--og-color-success)', disabled: true});
            }
            else {
    ...
    protected startActionCountdown(challengeAsset: Asset) {
        console.log('startActionCountdown');
        const startAttribute = challengeAsset?.attributes['challengeStart']?.value;
        if(startAttribute) {
            clearInterval(this.countdownInterval);
            this.countdownInterval = setInterval(() => {
                if(moment().isAfter(startAttribute)) {
                    this.setActionVisual({text: i18next.t('panel_peakNotification.startingSoon'), color: 'var(--og-color-success)', disabled: true});
                    clearInterval(this.countdownInterval);
                } else {
                    const diff = moment(startAttribute).diff(moment(), 'seconds');
                    const text = diff > 60
                            ? i18next.t('panel_peakNotification.startingInFewMinutes', { 'time': moment.duration(diff, 'seconds').humanize() })
                            : i18next.t('panel_peakNotification.startingInFewSeconds', { 'time': moment.duration(diff, 'seconds').humanize() });
                    if(this.action.text !== text) {
                        this.setActionVisual({text: text, color: 'var(--og-color-success)', disabled: true});
                    }
                }
            }, 1000);
        } else {
    ...
    protected async getPanelContent(): Promise<TemplateResult> {
        return html`
            <div style="display: flex; flex-direction: column; gap: 24px;">
                ...
                ${until(this.getSummaryTemplate(this.meterAsset, this.challengeAsset))}
            </div>
        `;
    }
    ...
    public async onActionButtonClick() {
        console.log('onActionButtonClick');
        ...
        if(this.meterAsset && this.challengeAsset) {
            ...
            if(!readonly || this.status === OgMeterChallengeState.JOIN_CHALLENGE) {
                ...
                this.setActionVisual({text: i18next.t('panel_peakNotification_udg.joinedText'), color: 'var(--og-color-success)', disabled: true});
    ...
    protected async doJoinChallenge() {
        console.log('doJoinChallenge');
        this.waitingForChallengeDeferred = new Util.Deferred();
        console.log(this.meterAsset.id);
