...
export function pageEnergiesProvider(store: Store<GridAppStateKeyed>): OgPageProvider<AppStateKeyed> {
    return {
        name: 'energies',
        routes: ['energies'],
        pageCreator: () => new PageEnergies(store)
    };
}
...
@customElement('page-energies')
export class PageEnergies extends OgPage<GridAppStateKeyed> {
    ...
    protected willUpdate(changedProps: PropertyValues) {
        if(changedProps.has('meterAsset') && this.meterAsset) {
            ...
            const isInChallenge = false; //* connected && challengeStatus === OgMeterChallengeState.ACTIVE_CHALLENGE;
    ...
    protected render(): TemplateResult {
        return html`<div>Hola, aquesta es la pagina d'energies</div>`;
    }
}
