...
import {GridAppStateKeyed, setServiceUserData} from "../../util/og-state";
...
export class PageAddHomeAutomation extends OgPage<GridAppStateKeyed> {
    ...
    stateChanged(state: GridAppStateKeyed): void {
        ...
        const characteristics = JSON.parse(this.userAsset?.attributes?.[PageAddHomeAutomation.HOUSEHOLD_CHARACTERISTICS_ATTRIBUTE]?.value) as DeviceCharacteristic[] | undefined;
        ...
        return html`
            <div class="page-wrapper">
                ...
                <div class="page-content">
                    ${until(this._getHomeAutomationFormTemplate(), html`Loading...`)}
                </div>
                <og-input class="page-action" .type=${InputType.BUTTON} raised rounded fullWidth label="addHomeAutomation.addDevice"
        ...
        return html`
            <div class="page-content-container">
                <div class="homeautomation-option-item vertical">
                    ...
                    <og-input type=${InputType.SELECT} label=${i18next.t('addHomeAutomation.selectBrandPlaceholder')} style="width: 100%;"
    ...
    protected _onHomeAutomationAddClick(ev: OrInputChangedEvent) {
        if(this._isValid()) {
            ...
            switch (this._selectedBrand) {
                case OgHomeAutomationBrand.HOME_ASSISTANT: prefix = "homeassistant"; break;
                case OgHomeAutomationBrand.OPENHAB: prefix = "openhab"; break;
                default: prefix = undefined; break;
            }

            // Create a service user
            rest.api.UserAccountResource.createUpdateServiceUserAccount({ prefix: prefix }).then((response) => {
