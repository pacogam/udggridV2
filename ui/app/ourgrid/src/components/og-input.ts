...
import type { OrTranslate } from "@openremote/or-translate";
    ...
    if (this.type === InputType.BUTTON) {
      ...
      return html`
        <button
          ...
          @mousedown="${(ev: MouseEvent) => onMouseDown(ev)}"
          @mouseup="${(ev: MouseEvent) => onMouseUp(ev)}"
        >
          ${!isIconButton ? html` <div class="mdc-button__ripple"></div>` : ``}
          ${
            this.icon
              ? html` <or-icon
                  class="${isIconButton ? "" : this.action ? "mdc-fab__icon" : "mdc-button__icon"}"
                  aria-hidden="true"
                  icon="${this.icon}"
                ></or-icon>`
              : ``
          ...
          ${
            this.label || this.error
              ? html`
                  <span
                    class="${this.action ? "mdc-fab__label" : "mdc-button__label"}"
                    style="${this.loading ? "visibility: hidden" : nothing}"
                  >
                    ${
                      this.doTranslate
                        ? html` <or-translate .value="${this.error ? this.error : this.label}"></or-translate>`
                        : html`${this.error ? this.error : this.label}`
                    }
                  </span>
                `
              : ``
          ...
          ${
            !isIconButton && this.iconTrailing
              ? html` <or-icon
                  class="${this.action ? "mdc-fab__icon" : "mdc-button__icon"}"
                  aria-hidden="true"
        ...
        <div style="display: flex; gap: 8px;">
          ${this.options.map((item: string | OgInputButtonGroupOption, index) => {
            ...
            const buttonStyles: {} = {
              width: "34px",
              "border-radius": "50%",
              "border-color": !isString
                ? this.value === index
                  ? item.borderColors?.active
                  : item.borderColors?.inactive
                : undefined,
              "background-color": !isString
                ? this.value === index
                  ? item.fillColors?.active
                  : item.fillColors?.inactive
                : undefined,
            };
            const iconStyles: {} = {
              "--or-icon-fill": !isString
                ? this.value === index
                  ? item.iconColors?.active
                  : item.iconColors?.inactive
                : undefined,
            };
            return html`
              <button
                class="mdc-button mdc-button--outlined mdc-button-grouped"
                style="${styleMap(buttonStyles)}"
                @click="${(ev: MouseEvent) => onClick(ev, index)}"
              >
                ${when(
                  this.loading,
                  () => html`
                    <span style="position: absolute;"
                      ><og-loading
                        size="medium"
                        style="display: flex; --og-loading-color: var(--mdc-theme-primary);"
                      ></og-loading
                    ></span>
                  `,
                  () => html`
