 ...
 */
import { LitElement, type TemplateResult, type PropertyValues, html, css, unsafeCSS, nothing } from "lit";
      ...
      <div id="swipable-wrapper">
        <div style="display: flex; align-items: center;">
          ${when(
            ...
            () => html`
              <div
                class="swipable-arrow-container"
                style="opacity: ${this.isSliding || this.selected - 1 < 0 ? "0.3" : "1"}"
                @click="${() => this.goToPreviousPage()}"
              >
                ${when(true, () => html` <or-icon icon="chevron-left"></or-icon>`)}
              </div>
            `
          )}
          ...
          ${when(
            ...
            () => html`
              <div
                class="swipable-arrow-container"
                style="opacity: ${this.isSliding || this.selected + 1 === this.size ? "0.3" : "1"}"
                @click="${() => this.goToNextPage()}"
              >
                ${when(true, () => html` <or-icon icon="chevron-right"></or-icon>`)}
              </div>
            `
          )}
        </div>
        ...
        <!-- Bottom controls -->
        ${guard(
          [this.dots, this.size, this.dotsClickable, this.selected, this.language],
          () => html`
            ${when(
              this.dots,
              () => html`
                <div id="swipable-controls">
                  ${map(range(this.size), (index) => {
                    ...
                    return html` <div class="${classMap(classes)}" @click="${() => this.onDotClick(index)}"></div> `;
                  })}
                </div>
              `
            )}
