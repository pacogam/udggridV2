...
import manager, { DefaultColor4, type Languages } from "@openremote/core";
import type { Asset, Attribute, Challenge } from "model";
import moment from "moment";
import { until } from "lit/directives/until.js";
import { InputType, type OrMwcInput } from "@openremote/or-mwc-components/or-mwc-input";
      ...
      .setContent(
        () => html`
          <div class="text-primary bold">
            <or-translate
              style="color: var(--og-color-secondary)"
              value="${"panel_challengeMissed.text"}"
            ></or-translate>
          </div>
        `
      ) as OgDialog
...
export function showLastChallengeResultDialog(
  meterAsset: Asset,
  ...
  const challengeDuration: number =
    challengeAsset?.attributes?.[Constants.CHALLENGE_DURATION_ATTRIBUTE]?.value || Defaults.CHALLENGE_DURATION_MINUTES;
  ...
  const challengeInterval: number =
    challengeAsset?.attributes?.[Constants.CHALLENGE_POINT_INTERVAL_ATTRIBUTE]?.value ||
  ...
  const getDetailsHtml = async (): Promise<TemplateResult> => {
    ...
    return html`
      <div class="text-primary bold" style="display: flex; flex-direction: column; gap: 6px; align-items: center;">
        <span style="color: var(--og-color-neutral)"
          >${i18next.t("panel_challengeComplete.pointsEarned").replace("{{value}}", challenge?.points || 0)}</span
        >
        ...
        () => html`
          <div style="display: flex; flex-direction: column; gap: 24px; width: 100%;">
            ...
            <div style="display: flex; flex-direction: column; align-items: center; gap: 18px;">
              <div style="width: 100%;">
                <og-challenge-progress
                  dark
                  noLabel
                  noPadding
                  .meterAsset=${meterAsset}
                  .challengeAsset=${challengeAsset}
                  style="color: var(--og-color-primary)"
                />
              </div>
              <div style="width: 80%">
                <og-points
                  ...
                  lines="2"
                  centered
                  .progress="${challengePointAttr.value || 0}"
                  .max=${challengeMaxPointsAchievable}
                ></og-points>
              </div>
              <div style="width: 100%; display: flex; justify-content: center;">
                ${until(getDetailsHtml(), () => html` <og-loading></og-loading> `)}
              </div>
            </div>
          </div>
        `
...
export class OgDialog extends OrMwcDialog {
  ...
  protected render() {
    ...
    const bgStyling: {} = {
      "background-color": this.dark ? "var(--og-color-primary-dark)" : "var(--og-color-primary)",
    };
          ...
          <div class="mdc-dialog__surface" tabindex="0" style="${styleMap(bgStyling)}">
            ${
              typeof this.heading === "string"
                ? html` <h2 class="mdc-dialog__title" id="dialog-title" style="${styleMap(textStyling)}">
                    <or-translate value="${this.heading}"></or-translate>
                  </h2>`
                : this.heading
                  ? html`<span class="mdc-dialog__title" id="dialog-title">${this.heading}</span>`
                  : ``
            }
            ${
              this.content
                ? html`
                        ...
                        this.actions
                          ? this.actions.map((action) => {
                              return html` <div
                                ...
                                ?data-mdc-dialog-button-default="${action.default}"
                    ...
                    ${
                      this.actionBtn
                        ? html`
                            <div
                              style="position: absolute; bottom: 24px; width: 100%; display: flex; justify-content: center; z-index: 10;"
                            >
                              <div style="position: fixed; width: 70vw;">
                                <og-input
                                  ...
                                  .disabled="${this.actionBtn.disabled}"
                                  .label="${this.actionBtn.content}"
                      ...
                      ${
                        !this.actions
                          ...
                          : this.actions!.map((action, _index) => {
                              return html` <li class="mdc-list-item" data-mdc-dialog-action="${action.actionName}">
                                <span class="mdc-list-item__text">${action.content}</span>
                              </li>`;
                            })
                      }
                    </ul>
