 ...
 */
import { getListTemplate, type ListItem, ListType, OrMwcList } from "@openremote/or-mwc-components/or-mwc-list";
...
export class OgMwcList extends OrMwcList {
  ...
  protected render() {
    const content = !this.listItems
      ? html``
      : html`${this.listItems.map((listItem, index) =>
          this.getItemTemplate(
            listItem,
            index,
            Array.isArray(this.values) ? this.values : this.values ? [this.values] : [],
            this.type
          )
        )}`;
  ...
  protected getItemTemplate(
    item: OgListItem | null,
    index: number,
    selectedValues: any[],
    type: ListType,
    translate?: boolean,
    itemClickCallback?: (e: MouseEvent, item: OgListItem) => void
  ) {
    if (item === null) {
      // Divider
      ...
      if (secondaryText !== undefined) {
        textTemplate = html`
          <span class="mdc-list-item__text">
            <span class="mdc-list-item__primary-text text-primary ${this.dark ? "dark" : undefined}"
              >${translate ? i18next.t(text) : text}</span
            >
            <span class="mdc-list-item__secondary-text text-secondary ${this.dark ? "dark" : undefined}"
              >${translate ? i18next.t(secondaryText) : secondaryText}</span
            >
          </span>
        `;
      } else {
        if (type === ListType.RADIO) {
          textTemplate = html`<label
            class="mdc-list-item__text text-primary ${this.dark ? "dark" : undefined}"
            for="radio-item-${index + 1}"
            >${translate ? i18next.t(text) : text}</label
          >`;
        } else {
          textTemplate = html`<span
            class="mdc-list-item__text text-primary ${this.dark ? "dark" : undefined}"
            title="${translate ? i18next.t(text) : text}"
            >${translate ? i18next.t(text) : text}</span
          >`;
        }
      }
    ...
    return html`
      <li
        ...
        style="${listItem.styleMap ? styleMap(listItem.styleMap) : ""}"
        class="mdc-list-item ${isSelected ? selectedClassName : ""}"
        role="${ifDefined(role)}"
      ...
      >
        ...
        ${leftTemplate} ${textTemplate} ${rightTemplate}
      </li>
    `;
  }
}
