...
export class OgUsageChart extends OrChart {
  ...
  shouldUpdate(changedProps: Map<string, any>): boolean {
    ...
    if (
      (changedProps.has("_chart") || changedProps.has("powerPercentageDatapoints")) &&
      !!this.powerPercentageDatapoints &&
      !!this._chart
    ) {
      this.applyBackgroundStyling(
        this._chart,
        this.powerPercentageDatapoints[0],
        this.powerPercentageDatapoints[1],
        this.districtAsset?.attributes.powerImportCriticalPercentage?.value
      );
    }
  ...
  public render() {
    ...
    return html`
      <div id="chart-wrapper">
        <div id="chart-title">
          <span
            class="text-secondary translucent"
          ...
          ${when(
            ...
            () => html`
              <span class="text-secondary translucent" style="padding-left: 4px; white-space: nowrap;"
                >${`Max ${this._maxValue}W`}</span
              >
            `
          )}
        </div>
        <div style="flex: 1; padding-right: 12px;">${super.render()}</div>
      </div>
    `;
  }
  ...
  protected applyBackgroundStyling(
    chart: any,
    powerDatapoints: ValueDatapoint<any>[],
    predictedPoints: ValueDatapoint<any>[],
    threshold?: number
  ) {
   ...
   */
  protected async _loadPowerAttributeData(
    asset: Asset,
    attribute: Attribute<any>,
    color: string | undefined,
    from: number,
    ...
    if (asset.id && attribute.name && this.datapointQuery) {
      ...
      if (query.type == "lttb" && !query.amountOfPoints) {
        if (this._chartElem.clientWidth == 0) {
          console.error(
            "Could not grab width of the Chart for estimating amount of datapoints. Using 10 points instead."
          );
        }
  ...
  protected async _loadDistrictAttributeData(
    asset: Asset,
    attribute: Attribute<any>,
    color: string | undefined,
    from: number,
    ...
    if (moment().isBefore(to)) {
      const responsePredicted = await manager.rest.api.AssetPredictedDatapointResource.getPredictedDatapoints(
        this.districtAsset.id,
        "powerDistrict",
        {
          type: "interval",
