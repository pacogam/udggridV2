...
export class OgUsageGraphic extends LitElement {
  ...
  protected render(): TemplateResult {
    ...
    switch (this.type) {
      ...
      case GraphicType.HEADER: {
        ...
        return html` <img src="${headerFooterUrl}" style="width: 100%;" /> `;
      }
      // Footer image only, which is normally placed at the bottom of the screen.
      case GraphicType.FOOTER: {
        const headerFooterUrl = this.getHeaderFooterByColor(this.color, this.dark, this.small);
        return html` <img src="${headerFooterUrl}" style="width: 100%; rotate: 180deg;" /> `;
      }
      ...
      case GraphicType.HEADER_FOOTER: {
        ...
        return html`
          <div
            style="height: 100%; display: flex; flex-direction: column; justify-content: space-between; padding: 16px;"
          >
  ...
  protected getVideoTemplate(
    color: OgStateColor = OgStateColor.GREEN,
    dark = false,
    fill = false,
    animate = false
  ): TemplateResult {
