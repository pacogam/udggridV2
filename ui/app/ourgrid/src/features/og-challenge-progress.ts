...
import { type Asset, AssetDatapointIntervalQueryFormula, type ValueDatapoint } from "@openremote/model";
import manager, { Util } from "@openremote/core";
import moment from "moment";
import { until } from "lit/directives/until.js";
import { i18next } from "@openremote/or-translate";
import { map } from "lit/directives/map.js";
import { getStateColorByPowerValue, type OgStateColor } from "../util/util";
...
export class OgChallengeProgress extends LitElement {
  ...
  protected render() {
    ...
    const startAttributeValue =
      this.challengeAsset?.attributes?.[Constants.CHALLENGE_START_TIME_ATTRIBUTE]?.value ||
      moment().subtract(challengeDuration, "minutes");
    const endAttributeValue =
      this.challengeAsset?.attributes?.[Constants.CHALLENGE_END_TIME_ATTRIBUTE]?.value || moment();
      ...
      <div id="progress-wrapper">
        ...
        <div id="progress-content">
          <div id="progress-container" class=${classMap(containerClasses)}>
            ...
            ${when(
              remainingWidthPercentage > 0,
              () => html` <div style="height: 100%; flex: 0 0 ${remainingWidthPercentage}%;" /> `
            )}
          </div>
        </div>
      </div>
      ...
      try {
        ...
        return html`
          ...
          <div style="position: absolute; top: 0; width: calc(100% - 20px); height: 8px; display: flex;">
            ${map(powerData, (_data) => html` <div style="flex: 1; border-right: 1px solid black;"></div> `)}
          </div>
        `;
      } catch (e) {
        return html`<span
          class="text-tertiary"
          style="flex: 1; color: ${this.dark ? "var(--og-color-primary)" : "var(--og-color-primary-dark)"}"
          >${i18next.t("error.unknown")}</span
        >`;
      }
    } else {
      return html`<span
      ...
      <div style="height: 100%; flex: 0 0 4px; background: var(--og-color-neutral); position: relative;">
        ...
        <div class="cursor-bottom">
          ${when(
            ...
            () => html` <span class="text-tertiary">${moment().format("HH:mm")}</span> `
          )}
        </div>
      </div>
    `;
  ...
  protected getPowerDatapoints(
    meterId: string,
    type: "power" | "max",
    startTimestamp = moment().subtract(1, "hour").valueOf(),
    endTimestamp = moment().valueOf()
  ): Promise<ValueDatapoint<any>[]> {
