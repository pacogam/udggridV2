...
import { getHeatPumpBrandAppUrl, getVehicleBrandAppUrl, type OgHeatPumpBrand, type OgVehicleBrand } from "../util/util";
...
export class PanelChallengeTips extends OgDataPanel {
      ...
      <div style="padding: 24px 0;">
        <div style="display: flex; flex-direction: column; gap: 10px;">
          ${when(check(WellknownCharacteristics.BATTERY, false), () => {
            const automaticControl: boolean =
              this.batteryAsset?.attributes?.[this.BATTERY_AUTOMATIC_CONTROL_ATTRIBUTE_NAME]?.value || false;
            const challengeActionButton: boolean =
              this.batteryAsset?.attributes?.[this.BATTERY_ACTION_BUTTON_ATTRIBUTE_NAME]?.value || false;
            const isManuallyActivated: boolean = automaticControl !== challengeActionButton;
            const powerSetpoint: number =
              this.batteryAsset?.attributes?.[this.POWER_SET_POINT_ATTRIBUTE_NAME]?.value || 0;
            ...
            return html`
              ...
              >
                ...
                ${when(
                  ...
                  () => html`
                    <div
                      style="position: absolute; display: block; bottom: -24px; width: calc(100% - 48px); left: 24px;"
                    >
          ...
          ${when(check(WellknownCharacteristics.ELECTRIC_VEHICLE, false) || this.vehicleAsset, () => {
            const automaticControl: boolean =
              this.vehicleAsset?.attributes?.[this.EV_AUTOMATIC_CONTROL_ATTRIBUTE_NAME]?.value || false;
            const chargerCharacteristic: DeviceCharacteristic | undefined = characteristics.get(
              WellknownCharacteristics.VEHICLE_CHARGER
            );
            const vehicleCharacteristic: DeviceCharacteristic | undefined = characteristics.get(
              WellknownCharacteristics.ELECTRIC_VEHICLE
            );
            const brand =
              vehicleCharacteristic?.brand ||
              `vehicleBrands.${this.vehicleAsset?.attributes?.manufacturer?.value?.toUpperCase()}`;
            const brandUrl = this._getBrandAppUrl(vehicleCharacteristic, this.vehicleAsset);
            const hasButton =
              ((chargerCharacteristic && vehicleCharacteristic) || this.vehicleAsset) && brandUrl !== undefined;
            const wattsSaved =
              this.vehicleAsset?.attributes?.[this.EV_POWER_ATTRIBUTE_NAME]?.value ??
              chargerCharacteristic?.wattsSaved ??
              Defaults.TIPS_VEHICLE_CHARGER_WATT_SAVED;
            let label: string;
            if (automaticControl) {
            ...
            return html`
              ...
              >
                <div style="padding: ${hasButton ? "24px 24px 48px 24px" : "24px"};">
                  ${getStatisticTemplate(
                    "images/car-charging.svg",
                    true,
                    html`
                      <div style="display: flex; flex-direction: column;">
                        <span
                          class="statistic-medium"
                          style="color: ${automaticControl ? "var(--og-color-success)" : "var(--og-color-danger)"}"
                          >${`-${wattsSaved}W`}</span
                        >
                        <or-translate class="text-primary" value=${label}></or-translate>
                      </div>
                      ...
                      ${when(
                        ...
                        () => html`
                          <og-input
                            ...
                            label="panel_tips.action2-automatic"
                            fullWidth
                            raised
                            rounded
                            comfortable
                        ...
                        () => html`
                          <a href="${brandUrl}" target="_blank">
                            <og-input
                              .type="${InputType.BUTTON}"
                              .label="${`goToVehicleBrandApp_${brand}`}"
                              fullWidth
                              raised
                              rounded
                              comfortable
          ...
          ${when(check(WellknownCharacteristics.HEAT_PUMP), () => {
            const heatPumpCharacteristic: DeviceCharacteristic | undefined = characteristics.get(
              WellknownCharacteristics.HEAT_PUMP
            );
            ...
            return html`
              ...
              >
                <div style="padding: ${hasButton ? "24px 24px 48px 24px" : "24px"};">
                  ${getStatisticTemplate(
                    "images/house-temperature-white.svg",
                    true,
                    html`
                      <div style="display: flex; flex-direction: column;">
                        <span class="statistic-medium" style="color: var(--og-color-danger)">
                  ...
                  () => html`
                    ...
                    >
                      <a href="${brandUrl}" target="_blank">
                        <og-input
                          .type="${InputType.BUTTON}"
                          .label="${`goToHeatPumpBrandApp_${heatPumpCharacteristic.brand}`}"
                          fullWidth
                          raised
                          rounded
                          comfortable
          ...
          <div class="background-tint" style="padding: 24px;">
            ${getStatisticTemplate(
              "images/kettle-steam-outline.svg",
              true,
              html`
                <div style="display: flex; flex-direction: column;">
   ...
   */
  protected _getBatteryBtnTemplate(
    automaticControl: boolean,
    challengeActionButton: boolean,
  ...
  protected _getBrandAppUrl(characteristic?: DeviceCharacteristic, asset?: Asset): string | undefined {
    const store: "google" | "apple" | undefined = manager.console.isMobile
      ? manager.console.shellAndroid
        ? "google"
        : manager.console.shellApple
          ? "apple"
          : undefined
      : "google";
    switch (characteristic?.id) {
      case WellknownCharacteristics.ELECTRIC_VEHICLE:
        return getVehicleBrandAppUrl(characteristic.brand as OgVehicleBrand | undefined, store);
      case WellknownCharacteristics.HEAT_PUMP:
        return getHeatPumpBrandAppUrl(characteristic.brand as OgHeatPumpBrand | undefined, store);
      default:
        break;
    }
    switch (asset?.type) {
      case "OurgridVehicleAsset": {
        ...
        if (manufacturer)
          return getVehicleBrandAppUrl(
            `vehicleBrands.${manufacturer.toUpperCase()}` as OgVehicleBrand | undefined,
            store
          );
        break;
      }
  ...
  protected _setBatteryActionButtonAttribute(newState: boolean): void {
    if (this.meterAsset) {
      rest.api.DeviceBatteryResource.actionButton({ meterId: this.meterAsset.id, buttonState: newState })
        .then(() => {
          if (newState) {
