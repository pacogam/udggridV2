...
import type { TimePresetCallback } from "@openremote/or-chart";
...
import type { OgUsageChart } from "../features/og-usage-chart";
...
export class PanelUsageHistory extends OgDataPanel {
    ...
    this.options = [
      new Map([
        [
          "7daysago",
          (_date) => [
            moment(new Date()).subtract(7, "day").startOf("day").toDate(),
            moment(new Date()).subtract(7, "day").endOf("day").toDate(),
          ],
        ],
      ]),
      new Map([
        [
          "6daysago",
          (_date) => [
            moment(new Date()).subtract(6, "day").startOf("day").toDate(),
            moment(new Date()).subtract(6, "day").endOf("day").toDate(),
          ],
        ],
      ]),
      new Map([
        [
          "5daysago",
          (_date) => [
            moment(new Date()).subtract(5, "day").startOf("day").toDate(),
            moment(new Date()).subtract(5, "day").endOf("day").toDate(),
          ],
        ],
      ]),
      new Map([
        [
          "4daysago",
          (_date) => [
            moment(new Date()).subtract(4, "day").startOf("day").toDate(),
            moment(new Date()).subtract(4, "day").endOf("day").toDate(),
          ],
        ],
      ]),
      new Map([
        [
          "3daysago",
          (_date) => [
            moment(new Date()).subtract(3, "day").startOf("day").toDate(),
            moment(new Date()).subtract(3, "day").endOf("day").toDate(),
          ],
        ],
      ]),
      new Map([
        [
          "2daysago",
          (_date) => [
            moment(new Date()).subtract(2, "day").startOf("day").toDate(),
            moment(new Date()).subtract(2, "day").endOf("day").toDate(),
          ],
        ],
      ]),
      new Map([
        [
          "yesterday",
          (_date) => [
            moment(new Date()).subtract(1, "day").startOf("day").toDate(),
            moment(new Date()).subtract(1, "day").endOf("day").toDate(),
          ],
        ],
      ]),
      new Map([
        ["today", (_date) => [moment(new Date()).startOf("day").toDate(), moment(new Date()).endOf("day").toDate()]],
      ]),
      new Map([
        [
          "tomorrow",
          (_date) => [
            moment(new Date()).add(1, "day").startOf("day").toDate(),
            moment(new Date()).add(1, "day").endOf("day").toDate(),
          ],
        ],
      ]),
    ];
   ...
   */
  protected async getChartContent(
    index: number,
    selected = 0,
    options: Map<string, TimePresetCallback>,
    lazyLoad = true
    ...
    return html`
      <og-usage-chart
        slot="${index}"
        .timePresetOptions="${options}"
        style="pointer-events: none;"
        .assets="${this.meterAsset ? [this.meterAsset] : []}"
  ...
  protected tryUpdateChartValues(assetId: string, attributeName: string, startEndValue: [Date, Date]) {
    ...
    manager.rest.api.AssetDatapointResource.getDatapoints(assetId, attributeName, {
      ...
      const validPoint = result.data.find(
        (d) => d.y !== undefined && (this.currentMax === undefined || d.y > this.currentMax)
      );
      if (validPoint) {
    ...
    manager.rest.api.AssetDatapointResource.getDatapoints(assetId, attributeName, {
      ...
      const validPoint = result.data.find(
        (d) => d.y !== undefined && (this.currentMin === undefined || d.y < this.currentMin)
      );
      if (validPoint) {
