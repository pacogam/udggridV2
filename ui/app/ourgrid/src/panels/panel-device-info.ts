 ...
 */
import { type TemplateResult, html, type PropertyValues, css } from "lit";
...
export class PanelDeviceInfo extends OgDataPanel {
    ...
    return html`
      <div style="position: relative;">
        <div style="display: flex; align-items: center; gap: 24px;">
          <div>
            <or-icon
              icon="${this.meterAsset ? "power-plug" : "power-plug-off"}"
          ...
          <div>
            <div style="display: flex; flex-direction: column; justify-content: space-between; gap: 8px;">
              <span class="text-heading2" style="text-align: start;"
                >${this.meterAsset ? i18next.t("panel_deviceInfo.yourDevice") : i18next.t("panel_deviceInfo.noDeviceFound")}</span
              >
                      ...
                      <div style="display: flex; align-items: center; gap: 8px;">
                        ...
                        <span class="text-tertiary" style="text-align: start;">
                          ${when(
                            model,
                            () => model,
                            () => html`<or-translate value="panel_deviceInfo.unknownModel"></or-translate>`
                          )}
                        </span>
                      </div>
                      <div style="display: flex; align-items: center; gap: 8px;">
                        ...
                        <span class="text-tertiary" style="text-align: start;">
                          ${when(
                            deviceId,
                            () => deviceId,
                            () => html`<or-translate value="panel_deviceInfo.unknownID"></or-translate>`
                          )}
                        </span>
                      </div>
                      <div style="display: flex; align-items: center; gap: 8px;">
                        ...
                        <span class="text-tertiary" style="text-align: start;">
                          ${when(
                            ...
                            () => html`
                              ...
                              ${version}
                            `,
                            () => html` <or-translate value="panel_deviceInfo.unknownVersion"></or-translate> `
                          )}
                        </span>
                      </div>
                    `;
  ...
  protected promptDeviceDelete() {
    ...
    showDialog(
      new OgDialog()
        ...
        .setContent(html` ${i18next.t("panel_deviceInfo.removePromptText")} `)
  ...
  protected async removeDevice(deviceName: string, assetType: string): Promise<void> {
    ...
    await rest.api.DeviceResource.removeDevice({ deviceName, assetType });
