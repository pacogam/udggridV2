...
import { type GridAppStateKeyed, realmSelector, setServiceUserData } from "../util/og-state";
...
import { type Asset, type DeviceCharacteristic, type User, WellknownCharacteristics } from "model";
...
export class PageHomeAutomation extends OgPage<GridAppStateKeyed> {
  ...
  stateChanged(state) {
    ...
    const characteristics = JSON.parse(
      this.userAsset?.attributes?.[PageHomeAutomation.HOUSEHOLD_CHARACTERISTICS_ATTRIBUTE]?.value
    ...
    return html`
      <div class="page-wrapper">
        ...
        <div class="page-title">
          <or-translate
            class="text-heading"
            value="page-homeAutomation.heading"
            style="text-align: center; max-width: 65vw;"
          ></or-translate>
        </div>
        <div class="page-content">
          <div class="page-content-container">${until(promise, html`Loading...`)}</div>
        </div>
      </div>
    `;
  }
  ...
  protected async _getHomeAutomationContent(software?: OgHomeAutomationBrand): Promise<TemplateResult> {
    ...
    switch (software) {
      case OgHomeAutomationBrand.HOME_ASSISTANT:
        markdown = this._processMarkdown(this.__language === "nl" ? HomeAssistantMd_NL : HomeAssistantMd_EN);
        break;
      case OgHomeAutomationBrand.OPENHAB:
        markdown = this._processMarkdown(this.__language === "nl" ? OpenHABMd_NL : OpenHABMd_EN);
        break;
      default:
        markdown = undefined;
        break;
    ...
    return html`
      ...
      ${when(
        this.serviceUser && markdown !== undefined,
        () => html`
