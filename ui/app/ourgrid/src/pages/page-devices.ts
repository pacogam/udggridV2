...
export class PageDevices extends OgPage<GridAppStateKeyed> {
    ...
    getAnimationEnterType = (oldPage: string) => {
        switch(oldPage) {
            case 'add-device': return PageAnimationType.SWIPE_RIGHT;
            default: return PageAnimationType.FADE;
        }
    }
            ...
            <div class="page-wrapper">
                ...
                <div class="page-action">
                    ${when(!removeButton, () => html`
                        <og-input .type=${InputType.BUTTON} raised rounded fullWidth label="deviceList.addDevice"
    ...
    protected async _getMeterTemplate(meterAsset = this.userAsset, language = this.language): Promise<TemplateResult> {
        return html`<panel-device-info .meterAsset="${meterAsset}" .language="${meterAsset}" @remove="${() => this._afterMeterRemove()}"></panel-device-info>`
    }
