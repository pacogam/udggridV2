...
import type { User } from "@openremote/model";
...
export class PageAccount extends OgPage<GridAppStateKeyed> {
    ...
    switch (ev.detail[0].value) {
      ...
      case "delete-account": {
        ...
        const dialogActions: OgDialogAction[] = [
          { actionName: "cancel", content: "cancel" },
          { actionName: "delete", content: "delete", action },
        ];
        showDialog(
          new OgDialog()
            ...
            .setContent(html`
              <or-translate
                value="page-account.deleteAccountConfirm"
                style="white-space: pre-line; margin-top: -1.5rem;"
              ></or-translate>
            `)
        ...
        <div style="display: flex; flex-direction: column; align-items: center;">
          ...
          >
            ...
            <span class="text-heading" style="margin-top: -10%; text-align: center; max-width: 65vw;"
              >${i18next.t("page-account.heading")}</span
            >
          </div>
          <div style="width: 100%;">
            <div>
  ...
  protected deleteAccount(userId?: string) {
    ...
    if (!userId || !realmName) {
      ...
      rest.api.UserAccountResource.deleteAccount()
        .then((r) => {
          if (r.status === 200) {
            manager.logout();
          }
