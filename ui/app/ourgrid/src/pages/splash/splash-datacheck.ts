...
import { type Asset, ClientRole, type User } from "@openremote/model";
...
export class SplashDatacheck extends OgSplashPage {
  ...
  getActionsTemplate(): TemplateResult {
    return html`
      <og-input
        id="loading-button"
        style="width: 100%; --or-mwc-input-color: var(--og-color-danger)"
        .type="${InputType.BUTTON}"
        label="tryAgain"
        fullWidth
        unElevated
        rounded
        @or-mwc-input-changed="${() => this.onTryAgain()}"
      ></og-input>
      <og-input
        id="logout-button"
        style="width: 100%; --or-mwc-input-color: var(--og-background-tint); --or-icon-fill: var(--og-color-secondary)"
        .type="${InputType.BUTTON}"
        icon="logout"
        label="logout"
        fullWidth
  ...
  protected async doChecks() {
    ...
    try {
      ...
      console.warn(e);
      if (
        e instanceof NoAssetLinkedError ||
        e instanceof NeedsOnboardingError ||
        e instanceof RequiresPrivacyConfirmationError
      ) {
  ...
  protected async fetchVehicleAsset(userId: string, delay?: number): Promise<Asset> {
    if (!this.vehicleAsset) {
      const assets = (
        await rest.api.AssetResource.queryAssets({ userIds: [userId], types: ["OurgridVehicleAsset"], limit: 1 })
      ).data;
