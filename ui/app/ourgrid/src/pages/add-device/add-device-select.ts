...
export class AddDeviceSelect extends OgPage<GridAppStateKeyed> {
    ...
    getAnimationEnterType = (oldPage) => {
        switch (oldPage) {
            case 'add-ev': return PageAnimationType.SWIPE_RIGHT;
            case 'add-heatpump': return PageAnimationType.SWIPE_RIGHT;
            case 'add-battery': return PageAnimationType.SWIPE_RIGHT;
            default: return PageAnimationType.SWIPE_LEFT;
        }
    }
    getAnimationExitType = (newPage) => {
        switch (newPage) {
            case 'add-ev': return PageAnimationType.SWIPE_LEFT;
            case 'add-heatpump': return PageAnimationType.SWIPE_LEFT;
            case 'add-battery': return PageAnimationType.SWIPE_LEFT;
        ...
        return html`
            <div class="page-wrapper">
                ...
                <div class="page-title">
                    <or-translate class="text-heading" value="addDevice.intro" style="text-align: center; max-width: 65vw;"></or-translate>
                </div>
            ...
            ${map(items, (item) => html`
                <div class="add-device-menu-item" @click=${() => this._onMenuSelect(item)}>
                    <div class="add-device-menu-item-icons">
                        ${when(item.icon, () => html`
                            <or-icon icon=${item.icon}></or-icon>`)}
                        ${when(item.svg, () => html`
                            <div class="svg-icon" style="mask-image: url(${item.svg})"></div>`)}
                    </div>
