...
import type { OgSwipable } from "../../components/og-swipable";
...
export abstract class OgOnboardingPage extends OgPage<GridAppStateKeyed> {
    ...
    return html`
      <div id="onboarding-wrapper">
        <div id="onboarding-container">
          ...
          ${when(
            !page.noTopGraphic || !!page.getHeading,
            () => html`
              <div id="onboarding-title" style="position: relative;">
                ...
                ${when(!!page.getHeading, () => {
                  return html`
                    <div
                      style="margin-top: -12%; --animate-offset: 0.2s; display: flex; flex-direction: column; max-width: 65vw; text-align: center; gap: 16px;"
                    >
          ...
          ${when(
            !page.noBottomGraphic || !!page.getActionText,
            () => html`
              <div id="onboarding-footer" style="${styleMap(footerStyling)}">
                ${when(
                  !page.noBottomGraphic,
                  () => html`
                    <og-usage-graphic
                      id="onboarding-bottomgraphic"
                      .type="${GraphicType.FOOTER}"
                      small="true"
                    ></og-usage-graphic>
                  `
                )}
                ${when(typeof page.getActionText === "function", () => {
                  ...
                  return html`
                    <og-input
                      ...
                      fullWidth
                      rounded
                      label="${page.getActionText()}"
                      .disabled="${disabled}"
                      .raised="${!disabled}"
                      .outlined="${disabled}"
                      style="--mdc-theme-primary: var(--og-color-warning); ${styleMap(buttonStyling)}"
                      @or-mwc-input-changed="${() => this.onActionClick(this.currentPageIndex)}"
                    ></og-input>
                  `;
                })}
              </div>
        ...
        () => {
          return html`
            <og-swipable
              ...
              .selected="${this.currentPageIndex}"
              @slide="${(ev) => this.onSlide(ev)}"
            >
              ${this.getOnboardingPagesContent(pages)}
            </og-swipable>
          `;
        },
        () => html` ${until(page.pageContent(), html` <og-loading></og-loading> `)} `
      )}
    `;
  }
  ...
  protected getOnboardingPagesContent(pages: OnboardPage[]): TemplateResult {
    return html`
      ${guard(
        [pages, this.language, this.currentPageIndex],
        () => html`
          ${map(pages, (p, index) => {
            return html`
              <div slot="${index}" style="height: 100%; width: 100%; overflow: auto;">
                ${until(p.pageContent(), html` <og-loading></og-loading> `)}
              </div>
            `;
          })}
        `
