...
import { type AppConfig, DEFAULT_LANGUAGES, OrApp, type Page, router } from "@openremote/or-app";
import manager, { DefaultColor2, DefaultColor3, DefaultColor4 } from "@openremote/core";
import { updateMetadata } from "pwa-helpers/metadata";
import { when } from "lit/directives/when.js";
import { guard } from "lit/directives/guard.js";
import { repeat } from "lit/directives/repeat.js";
import { type PageMenu, pageMenuProvider } from "../pages/page-menu";
import { i18next } from "@openremote/or-translate";
import { showLanguageDialog } from "../components/og-dialog";
import {
  NeedsOnboardingError,
  NoAssetLinkedError,
  RequiresPrivacyConfirmationError,
  splashDataCheckProvider,
} from "../pages/splash/splash-datacheck";
import { type OgPage, type OgPageProvider, PageAnimationType } from "../pages/util/og-page";
import {
  assetsSelector,
  ...
  type GridAppStateKeyed,
  realmSelector,
  setLanguage,
  userAssetIdSelector,
} from "./og-state";
import type { Asset } from "@openremote/model";
import type { OgManager } from "./og-manager";
...
export class OgApp<S extends GridAppStateKeyed> extends OrApp<any> {
  ...
  stateChanged(state: GridAppStateKeyed) {
    ...
    if (!this._attributeSubscriptionId || hasReconnected) {
      if (
        !this._loading &&
        userAssetIdSelector(state) &&
        districtAssetIdSelector(state) &&
        challengeAssetIdSelector(state) &&
        state.gridApp.assets.length >= 3
      ) {
  ...
  protected shouldUpdate(changedProps: PropertyValues): boolean {
    ...
    if (changedProps.has("_page") && this._page) {
      ...
      if (!changedProps.get("_page") && !exclusions.includes(this._page)) {
        ...
        if (
          !this._assets ||
          this._assets.length === 0 || // if no assets are present
  ...
  protected updated(changedProps: Map<string, any>) {
    if (changedProps.has("_initialised") && this._initialised && this._mainContainerElem) {
      this._mainContainerElem.onscroll = () => {
        this._headerShown = this._mainContainerElem
          ? this._mainContainerElem.scrollTop <= Defaults.HIDE_HEADER_FROM_HEIGHT_PX
          : true;
      };
  ...
  protected render(): TemplateResult {
    ...
    if (!this._initialised) {
      ...
      return html`
        ...
        ${guard(
          [this.appConfig, this._headerShown, this._dark, this._initialised, this._loading, this._isMenuActive],
          () => html`
            ${when(
              !pageProvider?.hideHeader,
              () => html`
                <div id="header-container">
                  <og-header
                    style="transform: translateY(${this._headerShown ? "0" : "-100%"})"
                    .dark="${this._dark || this._isMenuActive}"
                    .menuActive="${this._isMenuActive}"
                    .loading="${!this._initialised || this._loading}"
                    @menu="${() => (this._isMenuActive = !this._isMenuActive)}"
                  ></og-header>
                </div>
              `
            )}
            ${guard([], () => html` ${pageMenuProvider(this._store).pageCreator()} `)}
          `
        )}
  ...
  protected subscribeAssets = async (realm: string, assetIds: string[], requestCurrent = false) => {
    ...
    try {
      ...
      const attributeSubscriptionId = await manager.events.subscribeAttributeEvents(
        assetIds,
        requestCurrent,
        (event) => {
