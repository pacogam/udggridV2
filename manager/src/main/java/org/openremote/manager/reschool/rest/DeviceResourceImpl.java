...
public class DeviceResourceImpl extends ManagerWebResource implements DeviceResource {
  ...
  private static final Set<Class<? extends Asset<?>>> ENODE_ASSET_TYPES =
      Set.of(OurgridChargerAsset.class, OurgridHvacAsset.class, OurgridVehicleAsset.class);
  ...
  public Response linkDevice(RequestParams requestParams, DeviceLinkDetails details) {
    ...
    Class<? extends Asset<?>> assetTypeClass =
        ValueUtil.getAssetClass(details.assetType)
            .orElseThrow(
                () ->
                    new WebApplicationException(
                        "Invalid asset type: " + details.assetType, BAD_REQUEST));
            ...
            new AssetQuery()
                ...
                .attributes(
                    new AttributePredicate(
                        OurgridMeterAsset.DEVICE_ID.getName(),
                        new StringPredicate(AssetQuery.Match.EXACT, true, details.deviceName))));
    if (asset == null) {
      throw new WebApplicationException(NOT_FOUND);
    }

    // If the device is already connected to a user...
    Collection<UserAssetLink> assetLinks =
        assetStorageService.findUserAssetLinks(asset.getRealm(), null, asset.getId());
    ...
    Collection<UserAssetLink> userLinksOfUser =
        assetStorageService.findUserAssetLinks(asset.getRealm(), getUserId(), null);
    String[] assetIds =
        userLinksOfUser.stream().map(l -> l.getId().getAssetId()).toArray(String[]::new);
    Collection<Asset<?>> linkedAssetsOfUser =
    ...
    if (!linkedAssetsOfUser.isEmpty()) {
      throw new WebApplicationException(
          "User is already linked to a '" + details.assetType + "' asset", CONFLICT);
    }

    // Link user to the asset
    assetStorageService.storeUserAssetLinks(
        List.of(new UserAssetLink(asset.getRealm(), getUserId(), asset.getId())));
  ...
  public Response removeDevice(RequestParams requestParams, DeviceLinkDetails details) {
    ...
    List<UserAssetLink> userAssetLinks =
        assets.stream()
            .map(asset -> new UserAssetLink(asset.getRealm(), getUserId(), asset.getId()))
            .toList();
    try {
