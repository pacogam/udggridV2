...
public class UserAccountResourceImpl extends ManagerWebResource implements UserAccountResource {
    ...
    public UserAccountResourceImpl(TimerService timerService, ManagerIdentityService identityService, AssetStorageService assetStorageService) {
    ...
    public User createUpdateServiceUserAccount(RequestParams requestParams, String prefix) {
        ...
        try {
            ...
            User serviceUser = identityService.getIdentityProvider().createUpdateUser(getAuthenticatedRealmName(), newUser, null, true);

            // Update the roles of the new service user account
            String[] realmRoles = identityService.getIdentityProvider().getUserRealmRoles(getAuthenticatedRealmName(), getUserId());
            identityService.getIdentityProvider().updateUserRealmRoles(serviceUser.getRealm(), serviceUser.getId(), realmRoles);

            // Update client roles of the new service user account
            String[] clientRoles = identityService.getIdentityProvider().getUserClientRoles(getAuthenticatedRealmName(), getUserId(), Constants.KEYCLOAK_CLIENT_ID);
            identityService.getIdentityProvider().updateUserClientRoles(serviceUser.getRealm(), serviceUser.getId(), Constants.KEYCLOAK_CLIENT_ID, clientRoles);

            // Update the linked assets
            List<UserAssetLink> assetLinks = assetStorageService.findUserAssetLinks(getAuthenticatedRealmName(), getUserId(), null);
            List<UserAssetLink> newAssetLinks = assetLinks.stream().map(l ->
                    new UserAssetLink(serviceUser.getRealm(), serviceUser.getId(), l.getId().getAssetId())).toList();
    ...
    protected List<User> queryServiceUserAccounts() {
        List<User> users = List.of(identityService.getIdentityProvider().queryUsers(new UserQuery()
          .realm(new RealmPredicate(getAuthenticatedRealmName()))
          .usernames(new StringPredicate().match(AssetQuery.Match.END).value(getUserId()))
        ));
