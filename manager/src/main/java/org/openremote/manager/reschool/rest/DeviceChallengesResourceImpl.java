...
public class DeviceChallengesResourceImpl extends ManagerWebResource
    implements DeviceChallengesResource {
  ...
  public DeviceChallengesResourceImpl(
    ...
    this.deviceChallengesService =
        new DeviceChallengesService(timerService, assetStorageService, datapointService);
  }

  @Override
  public Response joinChallenge(
      RequestParams params, DeviceJoinChallengeDetails joinChallengeDetails) {
    if (!isAuthenticated()) {
    ...
    if (meterChallengeStatus.isEmpty()
        || meterChallengeStatus.get() != OurgridMeterAsset.ChallengeStatusValueType.joinChallenge) {
      throw new WebApplicationException(FORBIDDEN);
    }
  ...
  public Challenge getCurrent(RequestParams params) {
    ...
    Optional<?> meterChallengeStatus = meterAsset.getChallengeStatus();
    Collection<OurgridMeterAsset.ChallengeStatusValueType> allowed =
        Arrays.asList(
            OurgridMeterAsset.ChallengeStatusValueType.activeChallenge,
            OurgridMeterAsset.ChallengeStatusValueType.joinedChallenge);
    if (meterChallengeStatus.isEmpty()
        || !allowed.contains(
            (OurgridMeterAsset.ChallengeStatusValueType) meterChallengeStatus.get())) {
      throw new WebApplicationException(NOT_FOUND);
    }
  ...
  public Response getHistory(
      RequestParams params, DeviceChallengeHistoryDetails challengeHistoryDetails) {
    if (!isAuthenticated()) {
      throw new WebApplicationException(UNAUTHORIZED);
    }
    ...
    try {
      return Response.ok(
              this.deviceChallengesService
                  .getMeterChallengeHistory(
                      meterAsset,
                      challengesAsset,
                      challengeHistoryDetails.startTimestamp,
                      challengeHistoryDetails.endTimestamp)
                  .toArray(new Challenge[0]))
          .build();

    } catch (Exception e) {
      LOG.warning(
          "Error getting meter challenge history for " + getUserId() + ": " + e.getMessage());
