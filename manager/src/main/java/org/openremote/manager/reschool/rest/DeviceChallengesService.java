...
public class DeviceChallengesService {
    ...
    public Collection<Challenge> getMeterChallengeHistory(ReschoolMeterAsset meterAsset, OurGridChallengesAsset challengesAsset, long startTime, long endTime) throws Exception {
        ...
        Collection<Challenge> allChallenges = this.getTotalChallengeHistory(challengesAsset, startTime, endTime);
        ...
        Collection<Date> pointTimestamps = pointDatapoints.stream().map(dp -> new Date(dp.getTimestamp())).toList();


        // Loop through all challenges from the past, and check whether the timer has reset for the meter.
        Collection<Challenge> joinedChallenges = allChallenges.stream()
                .filter(challenge -> pointTimestamps.stream()
                        .anyMatch(pointGainDate -> (pointGainDate.after(challenge.getStartDate()) && pointGainDate.before(challenge.getEndDate()))))
                .toList();
        ...
        return joinedChallenges.stream().map((Challenge challenge) -> {

            // Use the queried CHALLENGE_POINTS to figure out the amount of points gained during the challenge.
            ...
            long endTimeMax = challenge.getEndDate().getTime() + (60 * 1000); // Including a 60-second margin
            Double[] datapoints = pointDatapoints.stream()
                    .filter(dp -> dp.getTimestamp() >= startTimeMin && dp.getTimestamp() <= endTimeMax)
                    .sorted(Comparator.comparing(ValueDatapoint::getTimestamp))
                    .map(dp -> (Double) dp.getValue())
                    .toArray(Double[]::new);

            int pointsGained = (datapoints.length <= 1) ? 0 : (datapoints[datapoints.length - 1].intValue() - datapoints[0].intValue());

            Optional<Date> joinDate = joinDates.stream()
                    .filter(dp -> dp.getTimestamp() >= startTimeMin && dp.getTimestamp() <= endTimeMax)
            ...
            return challenge
                    ...
                    .setJoinDate(joinDate.orElse(null));
    ...
    public Collection<Challenge> getTotalChallengeHistory(OurGridChallengesAsset challengesAsset, long startTime, long endTime) throws RuntimeException {
        ...
        return IntStream.range(0, startTimestamps.size())
                .mapToObj(index -> new Challenge()
                        .setStartDate(new Date(Objects.requireNonNullElse(startTimestamps.get(index), 0L)))
