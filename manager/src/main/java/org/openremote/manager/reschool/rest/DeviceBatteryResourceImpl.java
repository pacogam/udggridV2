...
public class DeviceBatteryResourceImpl extends ManagerWebResource implements DeviceBatteryResource {
  ...
  public DeviceBatteryResourceImpl(
  ...
  public Response automaticControl(RequestParams params, AutomaticControlDetails details) {
    ...
    if (!batteryAsset.hasAttribute(OurgridBatteryAsset.ALLOW_AUTOMATIC_CONTROL_BUTTON)) {
      batteryAsset.addAttributes(
          new Attribute<>(OurgridBatteryAsset.ALLOW_AUTOMATIC_CONTROL_BUTTON));
    ...
    assetProcessingService.sendAttributeEvent(
        new AttributeEvent(
            batteryAsset.getId(),
            OurgridBatteryAsset.ALLOW_AUTOMATIC_CONTROL_BUTTON.getName(),
            details.automaticControl));
    assetProcessingService.sendAttributeEvent(
        new AttributeEvent(
            batteryAsset.getId(),
            OurgridBatteryAsset.ALLOW_DISCHARGING_BUTTON.getName(),
            details.automaticControl));
  ...
  public Response actionButton(RequestParams params, ActionButtonDetails details) {
    ...
    batteryAsset
        .getAttribute(OurgridBatteryAsset.ALLOW_DISCHARGING_BUTTON)
  ...
  /**
   * Function that queries the battery {@link Asset} from the database using {@link
   * AssetStorageService} Requires the user to be linked to the battery asset, and having the
   * meterId as parent ID. Throws a {@link WebApplicationException} if for example the user has no
   * access.
   */
  protected Asset<?> getBatteryById(String meterId) throws WebApplicationException {
    ...
    Collection<UserAssetLink> userLinksOfAsset =
        assetStorageService.findUserAssetLinks(
            batteryAsset.getRealm(), getUserId(), batteryAsset.getId());
