...
public class EarneEnodeProtocol extends AbstractEarneProtocol<EarneEnodeAgent> {
  ...
  protected boolean isValidConfiguration() {
    return super.isValidConfiguration()
        && Stream.of(getRabbitMqControlQueue(), getRabbitMqUpdateQueue())
            .noneMatch(String::isBlank);
  }

  @Override
  protected void doStart(Container container) {
    controlResponseMessageHandler =
        new EarneEnodeControlResponseMessageHandler(agent, assetService, assetStorageService);
    updateMessageHandler =
        new EarneEnodeUpdateMessageHandler(agent, assetService, assetStorageService);
    controlHandler =
        new EarneEnodeControlHandler(
            assetStorageService, clientEventService, messageBrokerService, this);
  ...
  private void declareDurableQueue(String queueName) throws IOException {
    ...
    arguments.put(
        "durable",
        true); // TODO: Test if it is really needed because queueDeclare has a durable parameter.
        ...
        (consumerTag, delivery) -> {
          ...
          if (isControlResponse) {
            try {
              ...
              LOG.info(
                  String.format(
                      "agentName='%s', agentId='%s'; Control response message not processed: '%s'; Exception: %s",
            ...
            try {
              ...
              LOG.info(
                  String.format(
                      "agentName='%s', agentId='%s'; Update message not processed: '%s'; Exception: %s",
                      agent.getName(), agent.getId(), message, e));
            }
          }
        };
  ...
  public void publishControlMessage(String message) throws IOException {
    if (channel != null && !controlQueue.isBlank()) {
      ...
      LOG.finer(
          () ->
              String.format(
                  "agentName='%s', agentId='%s'; Published control message: %s",
                  agent.getName(), agent.getId(), message));
    } else {
      LOG.finer(
          () ->
              String.format(
                  "agentName='%s', agentId='%s'; No RabbitMQ connection to publish control message",
