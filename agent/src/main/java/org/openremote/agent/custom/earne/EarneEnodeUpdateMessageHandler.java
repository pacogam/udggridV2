...
public class EarneEnodeUpdateMessageHandler
    extends AbstractEarneMessageHandler<EarneEnodeUpdateMessage> {

  public EarneEnodeUpdateMessageHandler(
  ...
  protected void handleMessage(EarneEnodeUpdateMessage updateMessage) {
    ...
    if (assetUpdate == null) {
      throw new IllegalArgumentException(
          "Received EARN-E Enode update message without asset update: " + updateMessage);
    }

    String deviceId = assetUpdate.deviceId; // Asset name is unique EARN-E Enode device ID
    if (deviceId == null || deviceId.isBlank()) {
      throw new IllegalArgumentException(
          "Received EARN-E Enode update message without deviceId: " + assetUpdate);
    }

    String meterDeviceId = assetUpdate.userId;
    if (meterDeviceId == null || meterDeviceId.isBlank()) {
      throw new IllegalArgumentException(
          "Received EARN-E Enode update message without userId: " + assetUpdate);
    }
    ...
    if (meterAsset == null) {
      LOG.fine(
          String.format(
              "Ignoring EARN-E Enode update message for unknown meter with deviceId '%s'",
              meterDeviceId));
      return;
    }
    ...
    switch (assetUpdate) {
      case ChargerUpdate chargerUpdate -> {
        OurgridChargerAsset chargerAsset =
            findOrCreateAsset(
                assetId,
                deviceId,
                parentId,
                ...
                () -> {
                  ...
                  asset.addOrReplaceAttributes(
                      new Attribute<>(OurgridChargerAsset.ALLOW_AUTOMATIC_CONTROL_BUTTON, true));
                  return asset;
                });
        updateChargerAsset(assetId, deviceId, updateMessage, chargerAsset);
      }
      case HvacUpdate hvacUpdate -> {
        OurgridHvacAsset hvacAsset =
            findOrCreateAsset(
                assetId,
                deviceId,
                parentId,
                ...
                () -> {
                  ...
                  asset.addOrReplaceAttributes(
                      new Attribute<>(OurgridHvacAsset.ALLOW_AUTOMATIC_CONTROL_BUTTON, true));
                  return asset;
                });
        updateHvacAsset(assetId, deviceId, updateMessage, hvacAsset);
      }
      case VehicleUpdate vehicleUpdate -> {
        OurgridVehicleAsset vehicleAsset =
            findOrCreateAsset(
                assetId,
                deviceId,
                parentId,
                ...
                () -> {
                  ...
                  asset.addOrReplaceAttributes(
                      new Attribute<>(OurgridVehicleAsset.ALLOW_AUTOMATIC_CONTROL_BUTTON, true));
                  return asset;
                });
      ...
      default ->
          throw new IllegalArgumentException(
              "Received EARN-E Enode update message with unsupported asset update: " + assetUpdate);
    }
  }
  ...
  private OurgridMeterAsset findMeterAsset(String deviceId) {
    ...
    return (OurgridMeterAsset)
        assetService.findAssets(assetQuery).stream().findFirst().orElse(null);
  }
  ...
  private void copyParentUserAssetLinks(String parentId, String assetId) {
    ...
    List<UserAssetLink> parentUserAssetLinks =
        assetStorageService.findUserAssetLinks(agent.getRealm(), null, parentId);
    List<UserAssetLink> userAssetLinks =
        parentUserAssetLinks.stream()
            .map(link -> new UserAssetLink(agent.getRealm(), link.getId().getUserId(), assetId))
            .toList();
    assetStorageService.storeUserAssetLinks(userAssetLinks);
  }

  private <T extends Asset<?>> T findOrCreateAsset(
      String assetId,
      String deviceId,
      String parentId,
      EarneEnodeUpdateMessage updateMessage,
      Supplier<T> newAssetSupplier) {
    ...
    LOG.info(
        String.format(
            "agentName='%s', agentId='%s'; Created %s: '%s'",
            agent.getName(), agent.getId(), asset.getAssetType(), deviceId));
    return asset;
  }

  private void updateChargerAsset(
      String assetId,
      String deviceId,
      EarneEnodeUpdateMessage updateMessage,
      OurgridChargerAsset chargerAsset) {
    ...
    sendAttributeEvent(
        assetId, OurgridChargerAsset.START_CHARGING_CAPABLE, capabilities.startCharging.isCapable);
    sendAttributeEvent(
        assetId, OurgridChargerAsset.STOP_CHARGING_CAPABLE, capabilities.stopCharging.isCapable);
    sendAttributeEvent(
        assetId, OurgridChargerAsset.SUPPORTS_EXPORT, capabilities.startCharging.isCapable);
    ...
    LOG.finer(
        () ->
            String.format(
                "agentName='%s', agentId='%s'; Updated Charger Asset: '%s'",
                agent.getName(), agent.getId(), deviceId));
  }

  private void updateHvacAsset(
      String assetId,
      String deviceId,
      EarneEnodeUpdateMessage updateMessage,
      OurgridHvacAsset hvacAsset) {
    ...
    LOG.finer(
        () ->
            String.format(
                "agentName='%s', agentId='%s'; Updated HVAC Asset: '%s'",
                agent.getName(), agent.getId(), deviceId));
  }

  private void updateVehicleAsset(
      String assetId,
      String deviceId,
      EarneEnodeUpdateMessage updateMessage,
      OurgridVehicleAsset vehicleAsset) {
    ...
    if (capabilities.chargeState.isCapable && chargeState != null) {
      ...
      if (chargeState.batteryLevel != null) {
        sendAttributeEvent(
            assetId,
            OurgridVehicleAsset.ENERGY_LEVEL_PERCENTAGE,
            Math.toIntExact(chargeState.batteryLevel));
      }
      if (chargeState.chargeLimit != null) {
        sendAttributeEvent(
            assetId, OurgridVehicleAsset.CHARGE_LIMIT, Math.toIntExact(chargeState.chargeLimit));
      }
      if (chargeState.chargeRate != null) {
        sendAttributeEvent(
            assetId, OurgridVehicleAsset.POWER, Double.valueOf(chargeState.chargeRate));
      }
      if (chargeState.chargeTimeRemaining != null) {
        sendAttributeEvent(
            assetId,
            OurgridVehicleAsset.CHARGE_TIME_REMAINING,
            Math.toIntExact(chargeState.chargeTimeRemaining));
      }
      if (chargeState.range != null) {
        sendAttributeEvent(
            assetId, OurgridVehicleAsset.MILEAGE_CHARGED, Double.valueOf(chargeState.range));
      }
    }
    ...
    if (capabilities.location.isCapable && location != null) {
      if (updateAssetLocation(vehicleAsset, location.longitude, location.latitude)) {
        sendAttributeEvent(
            assetId, OurgridVehicleAsset.LOCATION, vehicleAsset.getLocation().orElse(null));
      }
    }
    ...
    if (capabilities.smartCharging.isCapable && smartChargingPolicy != null) {
      sendAttributeEvent(
          assetId, OurgridVehicleAsset.SMART_CHARGING_ENABLED, smartChargingPolicy.isEnabled);
    }

    sendAttributeEvent(assetId, OurgridVehicleAsset.SUPPORTS_IMPORT, true);
    sendAttributeEvent(
        assetId, OurgridVehicleAsset.START_CHARGING_CAPABLE, capabilities.startCharging.isCapable);
    sendAttributeEvent(
        assetId, OurgridVehicleAsset.STOP_CHARGING_CAPABLE, capabilities.stopCharging.isCapable);
    ...
    LOG.finer(
        () ->
            String.format(
                "agentName='%s', agentId='%s'; Updated Vehicle Asset: '%s'",
                agent.getName(), agent.getId(), deviceId));
  }
}
