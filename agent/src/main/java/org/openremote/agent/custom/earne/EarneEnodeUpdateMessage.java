...
public class EarneEnodeUpdateMessage {
    ...
    public static abstract class AbstractAssetUpdate {
        @JsonProperty("id")
        ...
        public String toString() {
            return addToStringJoiner(new StringJoiner(", ", getClass().getSimpleName() + "[", "]")).toString();
        }
    }
    ...
    public AbstractAssetUpdate getAssetUpdate() {
        List<AbstractAssetUpdate> updates = Stream.of(charger, hvac, vehicle).filter(Objects::nonNull).toList();
        if (updates.size() != 1) {
            throw new IllegalStateException("Expected exactly one asset update, got " + updates.size() + ": " + this);
        }
