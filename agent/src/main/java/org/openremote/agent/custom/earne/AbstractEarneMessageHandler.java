...
/**
 * Abstract base class for handling EARN-E JSON messages. Deserializes JSON messages into a specific
 * message type and provides methods to handle the message.
 *
...
public abstract class AbstractEarneMessageHandler<M> {
  ...
  public AbstractEarneMessageHandler(
  ...
  public void handleMessage(String json) {
    ...
    LOG.finer(
        () ->
            String.format(
                "agentName='%s', agentId='%s'; Handling %s: '%s'",
                agent.getName(), agent.getId(), messageName, json));
    ...
    LOG.fine(
        () ->
            String.format(
                "agentName='%s', agentId='%s'; Deserialized %s: '%s'",
                agent.getName(), agent.getId(), messageName, message));
  ...
  protected void updateAssetIds(Asset<?> asset, String assetId, String deviceId, String parentId) {
    if (!(asset instanceof OurgridDeviceAsset deviceAsset)) {
      throw new IllegalArgumentException(
          "Asset must be of type " + OurgridDeviceAsset.class.getSimpleName());
    }
    ...
    if (parentId != null && !parentId.isBlank()) {
      ...
      if (parentAsset != null) {
        ...
        LOG.finer(
            () ->
                String.format(
                    "agentName='%s', agentId='%s'; Parent asset '%s' not found",
                    agent.getName(), agent.getId(), parentId));
      ...
      LOG.finer(
          () ->
              String.format(
                  "agentName='%s', agentId='%s'; No parent asset provided for asset '%s'",
                  agent.getName(), agent.getId(), asset.getId()));
      asset.setParentId(agent.getId());
    }
  }
    ...
    if (longitude != null && latitude != null) {
      if (longitude != 0.0 && latitude != 0.0) {
        ...
        LOG.finer(
            () ->
                String.format(
                    "agentName='%s', agentId='%s'; Invalid coordinates provided for asset '%s': longitude=%f, latitude=%f",
                    agent.getName(), agent.getId(), asset.getId(), longitude, latitude));
      }
    } else {
      LOG.finer(
          () ->
              String.format(
                  "agentName='%s', agentId='%s'; No coordinates provided for asset '%s'",
                  agent.getName(), agent.getId(), asset.getId()));
    }
    return false;
  }
