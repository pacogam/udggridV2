...
public class EarneMeterMessageHandler extends AbstractEarneMessageHandler<EarneMeterMessage> {
  ...
  public EarneMeterMessageHandler(
      EarneMeterAgent agent,
  ...
  protected void handleMessage(EarneMeterMessage meterMessage) {
    ...
    if (deviceId == null || deviceId.isBlank()) {
      throw new IllegalArgumentException(
          "Received EARN-E Enode update message without deviceId: " + meterMessage);
    }
  ...
  private OurgridMeterAsset createMeterAsset(String assetId, EarneMeterMessage meterMessage) {
    ...
    meterAsset.setDeviceId(
        meterMessage
            .deviceId); // Unique Device ID will be used to connect between EARN-E and OpenRemote
    // app
    ...
    LOG.info(
        String.format(
            "agentName='%s', agentId='%s'; Created Meter Asset: %s",
            agent.getName(), agent.getId(), deviceId));
    return meterAsset;
  }

  private void updateMeterAsset(
      String assetId, EarneMeterMessage meterMessage, OurgridMeterAsset meterAsset) {
    String deviceId = meterMessage.deviceId;
    ...
    if (energyDeliveredTariff1 != null && energyDeliveredTariff2 != null) {
      energyImported =
          Math.round((energyDeliveredTariff1 + energyDeliveredTariff2) * 1000.0)
              / 1000.0; // E_imported_current, rounded to 3 decimals
    }

    Double energyExported = null;
    if (energyReturnedTariff1 != null && energyReturnedTariff2 != null) {
      energyExported =
          Math.round((energyReturnedTariff1 + energyReturnedTariff2) * 1000.0)
              / 1000.0; // E_exported_current, rounded to 3 decimals
    }

    Double energyNet = null;
    if (energyImported != null && energyExported != null) {
      energyNet =
          Math.round((energyImported - energyExported) * 1000.0)
              / 1000.0; // dE_current, rounded to 3 decimals
    }
    ...
    String timestampPrevious =
        meterAsset
            .getAttribute(OurgridMeterAsset.TIMESTAMP)
    ...
    if (timestamp != null && timestampPrevious != null) {
      ...
      if (dt <= 10) {
        LOG.info(
            String.format(
                "agentName='%s', agentId='%s'; Meter assetName='%s'; Time between messages < 10 seconds: dt='%s'",
                agent.getName(), agent.getId(), meterAsset.getName(), dt));
      }

      Double gasDeliveredPrevious =
          meterAsset
              .getAttribute(OurgridMeterAsset.GAS_IMPORT_TOTAL)
              .flatMap(Attribute::getValue)
              .orElse(null);
      ...
      if (dt > 0 && gasDelivered != null && gasDeliveredPrevious != null) {
        ...
        double gasFlowRate =
            Math.round(((dGas / dt) * 60) * 1000.0)
                / 1000.0; // Gas flow rate (m3/min), rounded to 3 decimals
        ...
        if (dGas >= 0) {
          ...
          LOG.info(
              String.format(
                  "agentName='%s', agentId='%s'; Meter assetName='%s'; Gas-meter reset, negative dGas: '%s'",
                  agent.getName(), agent.getId(), meterAsset.getName(), dGas));
        }
      }
    }
    ...
    String modelPrevious =
        meterAsset
            .getAttribute(OurgridMeterAsset.SMARTMETER_MODEL)
            .flatMap(Attribute::getValue)
            .orElse("");
    if (!model.isBlank() && !modelPrevious.equals(model)) {
      ...
      LOG.info(
          String.format(
              "agentName='%s', agentId='%s'; Meter Asset='%s'; Smart-meter model changed to: '%s'",
              agent.getName(), agent.getId(), meterAsset.getName(), model));
    }

    String swVersion = meterMessage.swVersion;
    String swVersionPrevious =
        meterAsset
            .getAttribute(OurgridMeterAsset.SOFTWARE_VERSION)
            .flatMap(Attribute::getValue)
            .orElse("");
    if (!swVersion.isBlank() && !swVersionPrevious.equals(meterMessage.swVersion)) {
      ...
      LOG.info(
          String.format(
              "agentName='%s', agentId='%s'; Meter Asset='%s'; EARN-E device software version update: '%s'",
              agent.getName(), agent.getId(), meterAsset.getName(), swVersion));
    }

    if (timestamp == null) {
      LOG.info(
          String.format(
              "agentName='%s', agentId='%s'; Meter Asset='%s'; Missing timestamp in message, uniqueDeviceName: '%s', assetId: '%s'",
              agent.getName(), agent.getId(), meterAsset.getName(), deviceId, assetId));
    }

    sendAttributeEvent(
        assetId, OurgridMeterAsset.TIMESTAMP, timestamp == null ? "" : timestamp.toString());
    ...
    LOG.finer(
        () ->
            String.format(
                "agentName='%s', agentId='%s'; Updated Meter Asset: '%s'",
                agent.getName(), agent.getId(), deviceId));
  }
}
