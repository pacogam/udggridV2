...
/**
 * Sends EARN-E Enode control messages based on attribute and persistence events. E.g. during
 * challenges charging can be stopped/started based on an attribute on the charger/vehicle asset.
...
public class EarneEnodeControlHandler extends RouteBuilder {
  ...
  public static final List<Class<? extends Asset>> EARNE_ENODE_ASSET_CLASSES =
      List.of(OurgridChargerAsset.class, OurgridHvacAsset.class, OurgridVehicleAsset.class);
  ...
  public EarneEnodeControlHandler(
      AssetStorageService assetStorageService,
      ClientEventService clientEventService,
      MessageBrokerService messageBrokerService,
      EarneEnodeProtocol earneProtocol) {
  ...
  public void start() {
    AssetFilter<AttributeEvent> filter =
        new AssetFilter<AttributeEvent>()
            .setAssetClasses(EARNE_ENODE_ASSET_CLASSES)
            .setAttributeNames(
                OurgridChargerAsset.ALLOW_STOP_CHARGING_BUTTON.getName(),
                OurgridHvacAsset.ALLOW_STOP_HEATPUMP_BUTTON.getName(),
                OurgridVehicleAsset.ALLOW_STOP_CHARGING_BUTTON.getName());
    ...
    from(PERSISTENCE_TOPIC)
        ...
        .filter(
            exchange -> {
              ...
              return EARNE_ENODE_ASSET_CLASSES.stream()
                  .anyMatch(assetClass -> assetClass.isAssignableFrom(entityType));
            })
        .process(
            exchange -> {
              PersistenceEvent<Asset<?>> persistenceEvent =
                  (PersistenceEvent<Asset<?>>) exchange.getIn().getBody(PersistenceEvent.class);
              if (persistenceEvent.getCause() == PersistenceEvent.Cause.DELETE) {
                ...
                if (!userId.isBlank()) {
                  ...
                  LOG.warning(
                      () ->
                          String.format(
                              "Could not determine userId for unlinking %s %s",
                              asset.getAssetType(), asset.getId()));
                }
              }
            });
  }
  ...
  private void onAttributeEvent(Event event) {
    ...
    if (assetClass == OurgridChargerAsset.class
        && OurgridChargerAsset.ALLOW_STOP_CHARGING_BUTTON.getName().equals(attributeName)) {
          ...
          .ifPresent(
              value ->
                  withAttributeEvent(
                      attributeEvent,
                      (userId, deviceId) -> {
                        ChargingControlCommand command =
                            value ? ChargingControlCommand.STOP : ChargingControlCommand.START;
                        sendControlRequestMessage(
                            new ChargingControlRequestMessage(
                                userId, deviceId, Discipline.CHARGER, command));
                      }));
    } else if (assetClass == OurgridHvacAsset.class
        && OurgridHvacAsset.ALLOW_STOP_HEATPUMP_BUTTON.getName().equals(attributeName)) {
      LOG.finer(() -> "Allow stop heatpump button changed");
    } else if (assetClass == OurgridVehicleAsset.class
        && OurgridVehicleAsset.ALLOW_STOP_CHARGING_BUTTON.getName().equals(attributeName)) {
              ...
              value ->
                  withAttributeEvent(
                      ...
                      (userId, deviceId) -> {
                        ...
                        sendControlRequestMessage(
                            new ChargingControlRequestMessage(
                                userId, deviceId, Discipline.EV, command));
                      }));
    }
  }
  ...
  private void withAttributeEvent(AttributeEvent event, BiConsumer<String, String> consumer) {
    ...
    if (userId.isBlank()) {
      LOG.finer(
          () ->
              String.format(
                  "Cannot determine userId using parent asset (%s). Event: %s",
                  event.getParentId(), event));
    } else if (deviceId.isBlank()) {
      LOG.finer(
          () ->
              String.format(
                  "Cannot determine deviceId using asset (%s). Event: %s", event.getId(), event));
    } else {
  ...
  private void sendControlRequestMessage(EarneEnodeControlRequestMessage message) {
    try {
      ...
      throw new IllegalStateException(
          String.format("Could not serialize control request message to JSON: %s", message), e);
    } catch (IOException e) {
      throw new IllegalStateException(
          String.format("Could not publish control request message: %s", message), e);
    }
  }
}
